...
void PMTree::CountPermutations(std::shared_ptr<Node> node, int& count) const {
    if (node->children.empty()) {
        count = 1;
        return;
    }
    ...
    for (const auto& child : node->children) {
        int sub_count = 0;
        CountPermutations(child, sub_count);
        count += sub_count;
    }
}
...
bool FindNthPermutation(PMTree& tree,
    ...
    if (!node->children.empty()) {
        for (const auto& child : node->children) {
            ...
            int subtree_count = 0;
            tree.CountPermutations(child, subtree_count);

            if (current_num + subtree_count >= target_num) {
                if (FindNthPermutation(tree, child, path, result, current_num, target_num)) {
                    return true;
                }
            } else {
